...
#endif // if (SPX_TREE_HEIGHT * (SPX_D - 1)) > 64
...
__device__ void dev_set_tree_addr(uint32_t addr[8], uint64_t tree)
{
    ...
    #error Subtree addressing is currently limited to at most 2^64 trees
#endif // if (SPX_TREE_HEIGHT * (SPX_D - 1)) > 64
	dev_ull_to_bytes(&((unsigned char *)addr)[SPX_OFFSET_TREE], 8, tree );
} // dev_set_tree_addr
...
__device__ void dev_copy_subtree_addr(uint32_t out[8], const uint32_t in[8])
{
	out[0] = in[0];
	out[1] = in[1];
	out[2] = ((out[2] & 0xffffff00) | (in[2] & 0xff));
	// memcpy( out, in, SPX_OFFSET_TREE + 8 );
...
void set_keypair_addr(uint32_t addr[8], uint32_t keypair)
{
 ...
 #endif // if SPX_FULL_HEIGHT / SPX_D > 8
...
{
	dev_u32_to_bytes(&((unsigned char *)addr)[SPX_OFFSET_TREE_INDEX], tree_index );
} // set_tree_index
