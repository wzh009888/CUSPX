...
int main() {
    ...
    printf("-------------------\n");
    printf("Testing SPX algorithmic parallelism ...\n");
    printf("-------------------\n");
    ...
    face_ap_crypto_sign_keypair(test_pk, test_sk);
    int wrong = 0;
    ...
    for (i = 0; i < SPX_SIGNATURES; i++) {
        ...
        int r = -1;
        for (int j = 0; j < SPX_BYTES + SPX_MLEN; j++) {
            if (sm_tmp[j] != sm[j]) {
                r = j;
                break;
            }
        }
        printf("set each byte of optrand is 0xad, test sig if same... ");
        if (r == -1)
            printf("Same signature, successful.\n");
        else
            printf("Different signature, failed. loc = %d\n", r);
#endif
    ...
    for (i = 0; i < SPX_SIGNATURES; i++) {
        ...
        /* Test if signature is valid. */
        if (face_ap_crypto_sign_open(mout, &mlen, sm, smlen, pk)) {
            printf("  X verification failed!\n");
            ret = -1;
        } else {
            printf("    verification succeeded.\n");
        ...
        /* Test if signature is valid when validating in-place. */
        if (face_ap_crypto_sign_open(sm, &mlen, sm, smlen, pk)) {
            printf("  X in-place verification failed!\n");
            ret = -1;
        } else {
            printf("    in-place verification succeeded.\n");
