...
#define single_stream_multi_key_data_parallelism
// #define multi_stream_multi_key_data_parallelism
// #define single_stream_single_key_data_parallelism
// #define multi_stream_single_key_data_parallelism

#if defined(single_stream_multi_key_data_parallelism)
...
#elif defined(multi_stream_multi_key_data_parallelism)
// right at first paper revision in whole link SPX_128F
#define GPU_KEYGEN face_ms_mdp_crypto_sign_keypair
#define GPU_SIGN face_ms_mdp_crypto_sign
#define GPU_VERIFY face_ms_mdp_crypto_sign_open
#elif defined(single_stream_single_key_data_parallelism)
#define GPU_KEYGEN face_sdp_crypto_sign_keypair
#define GPU_SIGN face_sdp_crypto_sign
#define GPU_VERIFY face_sdp_crypto_sign_open
#elif defined(multi_stream_single_key_data_parallelism)
#define GPU_KEYGEN face_ms_sdp_crypto_sign_keypair
#define GPU_SIGN face_ms_sdp_crypto_sign
#define GPU_VERIFY face_ms_sdp_crypto_sign_open
...
int main() {
...
#if defined(single_stream_multi_key_data_parallelism)
    printf("single-stream multi-keypair data parallelism\n");
#elif defined(multi_stream_multi_key_data_parallelism)
    printf("multi-stream multi-keypair data parallelism\n");
#elif defined(single_stream_single_key_data_parallelism)
    printf("single-stream single-keypair data parallelism\n");
#elif defined(multi_stream_single_key_data_parallelism)
    printf("multi-stream single-keypair data parallelism\n");
#endif // if defined(single_stream_multi_key_data_parallelism)

#if defined(SPX_128S) || defined(SPX_128S) || defined(SPX_128S)
    printf("We recommend use F para sets to run data parallism\n");
#endif
    ...
    GPU_SIGN(sm, &smlen, m, SPX_MLEN, sk, num);
    ...
    /* Test if signature is valid when validating in-place. */
    if (GPU_VERIFY(test_sm, &mlen, test_sm, smlen, pk, num)) {
        printf("  X in-place verification failed!\n");
        ret = -1;
    } else {
